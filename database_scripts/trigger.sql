...
-- Prevent infinite hierarchical loop on category INSERT
...
BEGIN
    ...
    IF v_depth >= 100 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Error: System detected category hierarchy too deep (>100 levels)!';
    END IF;
END //
...
-- Enforce business constraints (Race condition safe) on INSERT
...
BEGIN
    ...
    IF v_current_stock IS NULL OR v_is_deleted = TRUE OR v_product_deleted = TRUE THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Business Rule Violation: Product or variant is deleted or does not exist!';
    END IF;

    IF NEW.quantity > v_current_stock THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Business Rule Violation: Ordered quantity exceeds available stock!';
    END IF;
END //

-- Enforce business constraints (Race condition safe) on UPDATE
    ...
    IF NEW.quantity > OLD.quantity OR NEW.variant_id <> OLD.variant_id THEN
        ...
        IF NEW.variant_id = OLD.variant_id THEN
            IF NEW.quantity - OLD.quantity > v_current_stock THEN
                SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Business Rule Violation: Updated quantity exceeds available stock!';
            END IF;
        ELSE
            IF NEW.quantity > v_current_stock THEN
                SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Business Rule Violation: Updated quantity exceeds available stock for new variant!';
            END IF;
        END IF;
    END IF;
END //
...
BEGIN
    IF NEW.variant_id = OLD.variant_id THEN
        UPDATE PRODUCT_VARIANT SET stock = stock + OLD.quantity - NEW.quantity WHERE variant_id = NEW.variant_id;
    ELSE
